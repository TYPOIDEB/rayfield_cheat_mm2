return {
    Init = function(ctx, Tab)
        ...
        local function refreshPlayers()
            clearAllPlayers()
            if state.players and ctx.Alive then
                for _, p in ipairs(Players:GetPlayers()) do
                    applyPlayer(p)
                end
            end
        ...
        local function bindPlayer(player)
            player.CharacterAdded:Connect(function()
                task.wait(0.5)
                if ctx.Alive then applyPlayer(player) end
            end)
        end
        ...
        task.spawn(function()
            while ctx.Alive do
                task.wait(0.4)
                if ctx.Alive and state.players then
                    for _, p in ipairs(Players:GetPlayers()) do
                        applyPlayer(p)
                    end
                end
        ...
        Tab:CreateSlider({
            ...
            CurrentValue = 0.55,
            Flag = "esp_fill",
            Callback = function(v) state.fillAlpha = v; refreshPlayers() end,
        })
        ...
        Tab:CreateSlider({
            ...
            CurrentValue = 0,
            Flag = "esp_outline",
            Callback = function(v) state.outlineAlpha = v; refreshPlayers() end,
        })
